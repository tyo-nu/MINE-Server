...
def test_adduct_db_search():
    meh = services.adduct_db_search(Adduct_search_params(config.test_db, 164.0937301, 0.002, ['M+H'], True))
    assert len(meh) == 4
    assert len(meh[1]) == 3
    print len(meh[1][2])
...
def test_pathway_search():
    ...
    assert len(services.pathway_search(Pathway_query_params(config.test_db, 'C1b443383bfb0f99f1afe6a37f3ff2dadc3dbaff1', 'C89b394fd02e5e5e60ae1e167780ea7ab3276288e', 3, True))) == 9
