package mineDatabaseServicesServer;
...
sub call_method {
    ...
    my $ctx = mineDatabaseServicesServerContext->new($self->{loggers}->{userlog},
    ...
    # Service mineDatabaseServices does not require authentication.
	...
	my $stderr = mineDatabaseServicesServerStderrWrapper->new($ctx, $get_time);
	$ctx->stderr($stderr);
...
sub get_method
{
    ...
    {
	$self->exception('NoSuchMethod',
			 "'$method' is not a valid method in service mineDatabaseServices.");
    }
...
package mineDatabaseServicesServerContext;

use strict;

=head1 NAME

mineDatabaseServicesServerContext
...
package mineDatabaseServicesServerStderrWrapper;
